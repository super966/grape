...
public class Triangle{
    ...
    public void cutf(Triangle t, List<Triangle> l) {
        if (t.point[0].getPos().getW() > 0 && t.point[1].getPos().getW() > 0 &&
            ...
            l.add(t);
            return;
        } else if (t.point[0].getPos().getW() < 0 && t.point[1].getPos().getW() < 0 &&
                t.point[2].getPos().getW() < 0) {
            return;
        } else {
            Vertex v1 = t.point[0];
    ...
    public List<Triangle>  draw(Matrix worldMatrix, Camera c, boolean LINE, boolean LIGHT) {
...
//        picTri.add(new Triangle(temp));
        ...
        for (Triangle triangle : lt) {
            ...
            for (Triangle triangle1 : picTri) {
                for (int j = 0; j < 3; j++) {
                    triangle1.point[j].matrixMul(viewpointMatrix);
                }
            }
        }
